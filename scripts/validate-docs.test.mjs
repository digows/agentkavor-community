...
test('validates the complete released corpus', async () => {
  ...
  assert.deepEqual(result, { locales: 7, pages: 105, releases: 8 })
...
test('rejects an incomplete locale', async () => {
  await withCorpus(async (sourceDirectory) => {
    ...
    await assert.rejects(validateDocumentation(sourceDirectory), /must contain exactly 15 cataloged Markdown files/)
  })
})
...
test('requires content changes to advance a correctly chained release ID', async () => {
  await withCorpus(async (proposedSourceDirectory) => {
    ...
    await writeFile(
      catalogPath,
      catalog
        .replace('releaseId: docs-2026-08-11.1', 'releaseId: docs-2026-08-11.2')
        .replace('previousReleaseId: docs-2026-08-07.1', 'previousReleaseId: docs-2026-08-11.1'),
    )
...
test('binds a product release to its complete approved English documentation projection', async () => {
  ...
  const releaseNotesSourcePath = join(temporaryDirectory, 'v1.3.0.md')
  try {
    const publicSource = await readFile(resolve('docs/en/release-notes/1.3.0.md'), 'utf8')
    ...
    await assert.doesNotReject(validateProductReleaseProjection(resolve('docs'), {
      ...
      expectedReleaseId: 'docs-2026-08-11.1',
    }))
